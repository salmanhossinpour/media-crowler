...
| :--- | :--- | :--- | :--- |
| `GET` | `/api/movies/search` | `q` (الزامی), `source` (`all` \| `darknama` \| `cenamaflix`) | جستجوی فیلم و سریال بر اساس عنوان |
| `GET` | `/api/movies/latest` | `type` (`movie` \| `serie`), `count` (پیش‌فرض ۲۰) | آخرین عناوین اضافه شده همراه لینک‌های دانلود |
| `GET` | `/api/movies/seasons` | `id` (شناسه عددی سریال در دارک‌نما) | دریافت تمام فصل‌ها و اپیزودهای سریال با لینک دانلود |
| `GET` | `/api/movies/cenamaflix-post` | `url` (آدرس کامل پست در سینمافلیکس) | استخراج لینک‌های کیفیت‌های مختلف و پلیر آنلاین |
